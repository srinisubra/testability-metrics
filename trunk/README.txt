...
we are subtracting one is that we don't want to penalize you for breaking the
method into 2 smaller method. If the lowest cost would be 1 splitting the 
method into two would result in the cost of 1 twice. Having the simplest method
be 0 allows us to split the method to smaller functions which all have the cost
of 0. Hence the user is not penalized for breaking them methods into smaller
